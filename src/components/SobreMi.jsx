...
const SobreMi = memo(() => {
  ...
  const stats = [
    { value: '5+', label: language === 'es' ? 'Proyectos' : 'Projects', icon: <FolderGit2 size={14} /> },
    { value: '2022', label: language === 'es' ? 'Inicio' : 'Started', icon: <CalendarDays size={14} /> },
    { value: '3+', label: language === 'es' ? 'Años' : 'Years', icon: <Code2 size={14} /> },
    { value: '100%', label: 'Open Source', icon: <Monitor size={14} /> },
  ]
      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_1px_1fr] gap-y-10 md:gap-x-14 items-stretch md:min-h-[420px]">
          ...
          >
            ...
            <motion.div
              variants={stagger}
              className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-2 gap-2 mt-2"
            >
              {stats.map((stat) => (
                <motion.div
                  key={stat.value}

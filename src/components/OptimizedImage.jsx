...
const OptimizedImage = memo(({ src, alt, className, width, height, priority = 'low', sizes = "100vw", loading = 'lazy' }) => {
  ...
  const getOptimizedSrc = (baseSrc, targetWidth) => {
    ...
    if (baseSrc.includes('/images/profile')) {
      if (targetWidth <= 128) return `/images/profile-128.jpeg`;
      if (targetWidth <= 256) return `/images/profile-256.jpeg`;
    ...
    const availableSizes = [400, 640, 960, 1280, 1920];
  ...
  const getTargetWidth = () => {
    ...
    if (effectiveWidth < 128) return 128;
    if (effectiveWidth < 256) return 256;
  ...
  const generateSrcSet = () => {
    ...
    if (src.includes('/images/profile')) {
      return `
        /images/profile-128.jpeg 128w,
        /images/profile-256.jpeg 256w,
